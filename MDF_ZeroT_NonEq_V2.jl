...
function main(L::Int64,Nb::Int64,V::Float64,t::Float64)
    ...
    function Pt(t::Float64,L::Int64,N::Int64,U::Matrix{Float64},U2::Matrix{Float64},E2::Vector{Float64})::Matrix{ComplexF64}
        """
        ...
        """
        P = Matrix{Float64}(undef,L,N)
        P2 = Matrix{ComplexF64}(undef,L,N)

        BLAS.gemm!('T','N',1.0,U2,U[:,1:N],0.0,P)
        BLAS.gemm!('N','N',1.0+0.0im,complex.(U2),Dt2(E2,t).*P,0.0+0.0im,P2)
    ...
    function Pjt(j::Int64,L::Int64,N::Int64,Pt::Matrix{ComplexF64})::Matrix{ComplexF64}
        """
        Construct the matrix P of coefficients of an initial state (assuming
        ground state) with additional row corresponding to creation of a particle
        ...
        of Jordan-Wigner strings and particle creation at site j.
        """
        colj = Vector{ComplexF64}(undef,L)
        Pj = Matrix{ComplexF64}(undef,L,N+1)

        colj = zeros(Float64,L)
        colj[j] = 1.0
        Pj = O(j,L).*hcat(Pt,colj)

        return Pj
    end

    function Gijt(i::Int64,j::Int64,L::Int64,N::Int64,Pt::Matrix{ComplexF64})::ComplexF64
        """
        ...
        """
        local Gij::ComplexF64
        Pj = Matrix{ComplexF64}(undef,L,N)
        Pi = Matrix{ComplexF64}(undef,L,N)
        local Pij = Matrix{ComplexF64}(undef,N+1,N+1)

        Pi = Pjt(i,L,N,Pt)
        Pj = Pjt(j,L,N,Pt)
        BLAS.gemm!('C','N',1.0+0.0im,Pj,Pi,0.0+0.0im,Pij)
        Gij = det(Pij)
    ...
    function C(t::Float64,L::Int64,N::Int64,U::Matrix{Float64},U2::Matrix{Float64},E2::Vector{Float64},parity::Bool,TI::Bool)
        ...
        """
        # Ct0 = Matrix{Float64}(undef,L,L)
        Cmat = Matrix{ComplexF64}(undef,L,L)
        P = Matrix{ComplexF64}(undef,L,N)

        # Ct0::Matrix{ComplexF64} = NCorrZeroT(N,U) 
        Ct0::Matrix{ComplexF64} = NCorrZeroT_Mott(N,L) # Initialize Mott insulator
        Cmat = C_SFt(t,U2,E2,Ct0) 
        P = Pt(t,L,N,U,U2,E2)
        if TI==true
    ...
    E::Vector{Float64} = eigvals(TrapHamiltonian(L,1.0,0.1,V,false))
    U::Matrix{Float64} = eigvecs(TrapHamiltonian(L,1.0,0.1,V,false))
    ...
    # Ptest::Matrix{ComplexF64} = Pt(t,L,Nb,U,U2,E2)
    # # Ptest2::Matrix{ComplexF64} = Pjt(Int(L/2),L,Nb,Ptest)
    # @time Gijt(1,2,L,Nb,Ptest)
    

    print("HCB OBDM time: ")
    @time C_HCB::Matrix{ComplexF64} = C(t,L,Nb,U,U2,E2,true,false)
    open(string("HCB_free_expansion/C/C_L=",L,"_N=",Nb,"_V=",V,"_t=",t,"_trap.bin"),"w") do f
        write(f,C_HCB)
    end
    print("HCB MDF time: ")
    @time n_HCBxi::Vector{Float64} = real(BLAS.map(k->nkt(k,xi,C_HCB,sites),range(-pi,pi,L+1)));
    open(string("HCB_free_expansion/n/n_L=",L,"_N=",Nb,"_V=",V,"_t=",t,"_trap.bin"),"w") do f
        write(f,n_HCBxi)
    end
end

# main(400,50,1e-4,30.0)

for t::Float64 in range(0,150,4)
    main(1000,300,5*1e-3,t)
end