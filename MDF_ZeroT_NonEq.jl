"""
First Drafted: 21/07/25

Zero Temperature Post-quench Momentum Distribution Function for Hard Core Bosons.
"""
...
function main(L::Int64,Nb::Int64)
    ...
    function C(L::Int64,N::Int64,U::Matrix{Float64},parity::Bool)
        ...
        if parity==true
            Threads.@threads for i in range(1,L)
                ...
                Cmat[i,i+1:L-(i-1)] = [Gij(i,j,L,N,U) for j in range(i+1,L-(i-1))]
            end
            Threads.@threads for i in range(1,L)
    ...
    open(string("C_T=0_Equilibrium/C_L=",L,"_N=",Nb,"_free_PBC.bin"),"w") do f
        write(f,C_HCB)
    end
